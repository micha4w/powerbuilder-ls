...
void saveToFile(std::string path, const google::protobuf::Message& msg)
{
    std::string out;
    ...
    if (!status.ok())
        std::ofstream(OUT_DIR "/" + path + ".json-err") << "Failed to convert to json: " << status.message();
    else
        std::ofstream(OUT_DIR "/" + path + ".json") << out;

    // msg.SerializeToString(&out);
    // std::ofstream(OUT_DIR "/" + path + ".pb") << out;
}

#define DLLEXPORT extern "C" __declspec(dllexport)
DLLEXPORT DWORD WINAPI ExportStuff(POB_THIS obThis, int nArgCount)
{
    const auto retrieveFunction = [obThis](OB_CLASS_HNDL cls, OB_VTABLE_ID mid) -> std::optional<Function>
    {
        UINT no_args;
    ...
    for (UINT i = 0; i < count; i++)
    {
        ...
        {
            std::optional<Function> func = retrieveFunction(classes[i].class_hndl, mids[j]);
            if (func.has_value())
        ...
        for (UINT j = 0; j < no_items; j++)
        {
            std::optional<Function> func = retrieveFunction(classes[i].class_hndl, eids[j]);
            if (func.has_value())
    ...
    saveToFile("classes", out_classes);
    ...
    {
        std::optional<Function> func = retrieveFunction(hndl, mids[j]);
        if (func.has_value())
            *functions.add_function() = std::move(func.value());
    }

    saveToFile("functions", functions);
