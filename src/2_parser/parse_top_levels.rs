...
impl<I: Iterator<Item = char>> Parser<I> {
    ...
    pub fn parse_event_header(&mut self) -> EOFOrParserResult<(Event, Position)> {
    ...
    pub fn parse_function_header(&mut self) -> EOFOrParserResult<(Function, Position)> {
        ...
        if err.is_none() {
            if let Some(token) = self.optional(TokenType::Keyword(tokenizer::Keyword::THROWS))? {
                ...
                loop {
                    let (throw, res_err) = self.parse_type()?.split();
    ...
    pub fn parse_event(&mut self) -> EOFOr<Option<TopLevel>> {
        let (mut event, body_start) = ret_opt!(self.parse_event_header()?);
        event.help = self.previous_comments()?;
                ...
                TokenType::Keyword(tokenizer::Keyword::END) => {
                    match self.tokens.peek_nth(1)?.token_type {
                        TokenType::Keyword(tokenizer::Keyword::EVENT) => {
                            body_end = self.tokens.next()?.range.start;
                            end = self.tokens.next()?.range.end;
        ...
        Some(Some(TopLevel {
            ...
            top_level_type: TopLevelType::EventBody(EventBody {
                header: event,
                statements,
                body_range: Range {
                    start: body_start,
    ...
    pub fn parse_function(&mut self) -> EOFOr<Option<TopLevel>> {
        let (mut function, body_start) = ret_opt!(self.parse_function_header()?);
        function.help = self.previous_comments()?;
                    ...
                    match self.tokens.peek_nth(1)?.token_type {
                        TokenType::Keyword(
                                ...
                                ^ (function_type == tokenizer::Keyword::SUBROUTINE)
                            {
                                self.error(
                                    &if function.returns.is_none() {
                                        "Expected END SUBROUTINE, got END FUNCTION"
                                    } else {
                                        "Expected END FUNCTION, got END SUBROUTINE"
                                    }
                                    .into(),
                                    close.range,
                                );
        ...
        Some(Some(TopLevel {
            ...
            top_level_type: TopLevelType::FunctionBody(FunctionBody {
                header: function,
                statements,
                body_range: Range {
                    start: body_start,
    ...
    pub fn parse_on(&mut self) -> EOFOr<Option<TopLevel>> {
        ...
        let body_start = match self.expect_newline()? {
            Ok(newline) => newline.range.end,
            Err(_) => self.tokens.peek()?.range.start,
        };
        let body_end;
        ...
        Some(Some(TopLevel {
            top_level_type: TopLevelType::OnBody(OnBody {
                header: On {
                    class: DataType::simple(class),
                    name,
                },
                statements,
                body_range: Range {
    ...
    pub fn parse_datatype_decl(&mut self) -> EOFOr<Option<TopLevel>> {
        ...
        let base = base.unwrap_or_else(|| {
            DataType::simple(Token::fake_identifier(
                "powerobject".into(),
                Range::new_point(base_pos, self.uri()),
            ))
        });
        ...
        loop {
            match self.tokens.peek()?.token_type {
                ...
                TokenType::Keyword(tokenizer::Keyword::EVENT) => {
                    if let Some((event, _)) = self.parse_event_header()?.value() {
                        events.push(event);
                    }
                }
                TokenType::AccessType(_) | TokenType::Keyword(tokenizer::Keyword::FUNCTION) => {
                    if let Some((function, _)) = self.parse_function_header()?.value() {
    ...
    pub fn parse_scoped_variable_decl(&mut self) -> EOFOr<Option<TopLevel>> {
        ...
        if let StatementType::Declaration(vars) = variable.statement_type {
            if vars[0].access.read.is_some() || vars[0].access.write.is_some() {
                self.error(
                    &"File level variable cannot specify Access Rights".into(),
                    variable.range.clone(),
                );
            }
            for var in &vars {
                if let Some(expr) = &var.variable.initial_value {
                    self.error(
                        &format!(
                            "Freestanding file level variable cannot be instantiated directly, consider putting them into a {} VARIABLES block",
                            scope
                        ),
                        expr.range.clone(),
                    );
    ...
    pub fn parse_forward_decl(&mut self) -> EOFOr<Option<TopLevel>> {
        ...
        Some(Some(TopLevel {
            top_level_type: TopLevelType::ForwardDecl(ForwardDecl {
                variables: vars,
                classes: types,
            }),
            range: Range {
    ...
    pub fn parse_top_level(&mut self) -> EOFOr<Option<TopLevel>> {
        // TODO: catch eof and report error
        match self.tokens.peek()?.token_type {
            TokenType::Keyword(tokenizer::Keyword::FORWARD) => {
                match self.tokens.peek_nth(1)?.token_type {
                    TokenType::Keyword(tokenizer::Keyword::PROTOTYPES) => {
