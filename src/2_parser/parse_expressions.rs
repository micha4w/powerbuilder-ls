...
impl<I: Iterator<Item = char>> Parser<I> {
    ...
    pub fn parse_type(&mut self) -> EOFOrParserResult<DataType> {
        let (group, name) = ret_res!(self.parse_class_id()?);
        if let Some(g) = group {
            return Some(Ok(DataType::grouped(g, name)));
        }

        if name.content.eq_ignore_ascii_case("decimal")
            ...
            if self.optional(TokenType::Symbol(tokenizer::Symbol::LCURLY))?.is_some() {
                match self.optional(TokenType::Literal(tokenizer::Literal::NUMBER))? {
                    Some(prec) => {
                        ...
                        if let Err(err) = prec.content.parse::<usize>() {
                            self.error(
                                &format!("Invalid precision, has to be positive integer ({})", err),
                                prec.range.clone(),
                            );
                        } else {
                            return Some(Ok(DataType::with_curly(name, prec, end)));
                        }
                    }
                    None => {
                        let range = self.tokens.peek()?.range;
