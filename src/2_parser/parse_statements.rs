...
impl<I: Iterator<Item = char>> Parser<I> {
    ...
    pub fn parse_variable_declaration(
        ...
        loop {
                ...
                .is_some()
            {
                ...
                loop {
                    match self.tokens.peek()?.token_type {
                        ...
                        TokenType::Symbol(tokenizer::Symbol::COMMA) if !bounds.is_empty() => {
                            end = self.tokens.next()?.range.end;
                        }
                        TokenType::Literal(tokenizer::Literal::NUMBER) if bounds.is_empty() => {}
                        _ => {
                    ...
                    let Ok(first) = self.expect(TokenType::Literal(tokenizer::Literal::NUMBER))?
                    else {
                        break;
                    };
                    if self
                    ...
                    {
                        let Ok(second) =
                            self.expect(TokenType::Literal(tokenizer::Literal::NUMBER))?
                        else {
                            break;
                        };
                        ...
                        bounds.push(ArrayBounds::single(first));
                    }
                }
            ...
            let help = self.previous_comments()?;
        ...
        if let TokenType::Keyword(tokenizer::Keyword::CATCH) = self.tokens.peek()?.token_type {
            loop {
                ...
                let var = loop {
                    ...
                    break Some((data_type, name, help));
                };
    ...
    pub fn parse_call_statement(&mut self) -> EOFOr<Option<Statement>> {
        ...
        let call_type = match self.tokens.peek()?.token_type {
            ...
            TokenType::ID => {
                ...
                CallType::Ancestor(GroupedName::new(group, name))
            }
            _ => {
                return Some(
                    self.fatal(
        ...
        match self.tokens.peek()?.token_type {
            ...
            | TokenType::Keyword(
                ...
                TokenType::Symbol(tokenizer::Symbol::COLON) => {
                    ...
                    self.expect_newline();
                    Some(Some(Statement {
                        range: colon.range.merged(&token.range),
                        statement_type: StatementType::Label(token),
                    }))
                }
            ...
            TokenType::Keyword(tokenizer::Keyword::GOTO) => {
                let goto = self.tokens.next()?;
                let token = self.tokens.next()?;
                self.expect_newline();
                Some(Some(Statement {
                    range: goto.range.merged(&token.range),
                    statement_type: StatementType::Goto(token),
                }))
            }
            TokenType::Keyword(tokenizer::Keyword::EXIT) => {
                let range = self.tokens.next()?.range;
                self.expect_newline();
                Some(Some(Statement {
                    range,
            ...
            TokenType::Keyword(tokenizer::Keyword::CONTINUE) => {
                let range = self.tokens.next()?.range;
                self.expect_newline();
                Some(Some(Statement {
                    range,
