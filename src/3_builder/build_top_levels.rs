...
impl<'pars> Builder {
    fn build_scoped_vars(
        &mut self,
        vars: &'pars Vec<parser::ScopedVariable>,
    ) -> Vec<Arc<Variable<'pars>>> {
        vars.iter()
            .map(|var| {
                let new_var = Variable::new_scoped(var);
    ...
    fn build_function_declaration(
        &mut self,
        f: &'pars parser::Function,
    ) -> Arc<FunctionDeclaration<'pars>> {
                    ...
                    top_level_type: match &top_level.top_level_type {
                        ...
                        parser::TopLevelType::DatatypeDecl(datatype) => {
                            let variables = datatype
                                ...
                                .iter()
                                .map(|v| {
                                    let var = Arc::new(Variable::new_instance(v));
                        ...
                        parser::TopLevelType::FunctionBody(parsed) => {
                            let func = Arc::new(FunctionDefinition {
                                header: FunctionHeader::new(&parsed.header),
                                body: Body::empty(),
                        ...
                        parser::TopLevelType::EventBody(parsed) => {
                            let event = Arc::new(EventDefinition {
                                header: EventHeader::new(&parsed.header),
                                body: Body::empty(),
                    ...
                    TopLevelType::FunctionBody(function) => {
                        if let Some(class) = &mut class_ref {
                            ...
                            class
                                ...
                                .or_insert_with(|| {
                                    DefinitionDeclaration::definition(function.clone())
                                });
                        }
                    }
                    TopLevelType::EventBody(event) => {
                        if let Some(class) = &mut class_ref {
                            ...
                            class
                                ...
                                .or_insert_with(|| {
                                    DefinitionDeclaration::definition(event.clone())
                                });
                        }
                    }
            ...
            for top_level in &mut built.top_levels {
                match &mut top_level.top_level_type {
                    ...
                    TopLevelType::FunctionBody(def) => {
                        ...
                        entry
                            .map(|e| e.and_modify(|f| *f.get_definition_mut() = Some(def.clone())));
                    }
